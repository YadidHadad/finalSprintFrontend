<template>
    <section class="group-list flex">
        <Container orientation="horizontal" @drop="onDrop" group-name="group-lists" :get-child-payload="getChildPayload"
            ...
            <Draggable v-for="(group, i) in groupsCopy" :key="group.id">
                <group @addTask="addTask" @removeGroup="$emit('removeGroup', $event)" :group="group" :boardId="boardId"
                    :key="group.id" />
            </Draggable>
        </Container>
...
export default {
    ...
    methods: {
        onDrop(dropResult) {
            ...
            newGroups.then(data => {
                this.groupsCopy = JSON.parse(JSON.stringify(data))
            })
        },
